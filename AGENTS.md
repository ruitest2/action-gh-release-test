...
- Use `.github/workflows/repro-blocked-tag.yml` for blocked-tag finalization and orphan-draft cleanup behavior (`#722`).
  Configure `ACTION_GH_RELEASE_TRIGGER_TOKEN` first; the workflow defaults to this harness repository, requires the exact repository-rule cleanup diagnostic from the completed exercise job, directly verifies the diagnostic's release ID is absent, requires stable empty tag listings and a missing Git tag, and always cleans releases, tags, and the temporary ruleset.
