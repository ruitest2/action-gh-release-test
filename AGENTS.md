...
- Keep `v3.0.2` as the default released baseline in workflow inputs unless a newer released baseline intentionally replaces it.
- When testing a specific upstream PR or commit, override the workflow `action_ref` with the exact `softprops/action-gh-release` ref under test.
  ...
  - tag-push smoke on released `v3.0.2`
...
- Use `workflow_dispatch` on `main` and pass the exact upstream `action_repository` and `action_ref` under test.
  Unless you are testing a newer exact ref, the workflow defaults should stay on released upstream `v3.0.2`.
...
- Keep the default `e2e.yml` for simple smoke testing of release creation and asset upload on released upstream `v3.0.2`.
...
- Use `.github/workflows/repro-finalize-race.yml` for the draft-finalization retry path (`#704`, `#709`).
  Re-run both against released upstream `v3.0.2` or the exact upstream ref under test before opening a race-fix PR so the journal reflects which race still reproduces after the latest merged fixes.
...
- Use `.github/workflows/repro-blocked-tag.yml` for blocked-tag finalization and orphan-draft cleanup behavior (`#722`).
  Configure `ACTION_GH_RELEASE_TRIGGER_TOKEN` first. Use `control_mode: blocked-tag` for the successful regression verifier. Use `control_mode: invalid-action-token` for the expected failing control that checks out and executes the action after creating the ruleset, rejects the unrelated authentication failure, and still removes the ruleset. Both modes retain the original exercise attempt and always verify cleanup of releases, the tag, and the temporary ruleset.
...
- Use `.github/workflows/repro-draft-false.yml` for `draft: false` behavior when creating prereleases outside a tag-triggered job (`#253`, `#379`), for the four existing-draft prerelease transitions in upstream issue `#795` (omitted, null-expression, explicit false, and explicit true), and for moving-tag updates in upstream issue `#772`.
  Run `run_case: retag-existing-release` to seed a published prerelease, replace its Git tag without deleting the release, and verify that the same release ID receives the replacement asset without a duplicate draft or empty published release.
